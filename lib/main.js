    ...
    if (head[0] === "&" || head[0] === "|") {
      ...
      const callback = (error, result) => {
        if (error) {
          ...
          melf.emitter.post(alias+"/|"+token+"/"+JSON.stringify([error.name, error.message, lines.map(localize(melf.alias))]));
        } else {
          melf.emitter.post(alias+"/&"+token+"/"+JSON.stringify(result));
        }
      };
...
function rpcall (recipient, rpname, data, callback) {
  if (callback)
    return this.emitter.post(rpcallhelper(this, recipient, rpname, data, callback));
  let pending = true;
  let result = null;
  this.emitter.pull(rpcallhelper(this, recipient, rpname, data, (error, data) => {
    if (error) {
      error.stack = (
        error.name + ": " +
        error.message + "\n" +
